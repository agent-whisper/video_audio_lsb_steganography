...
def hide_secret(cover_video_dir, secret_msg_dir, key, output_file_name, lsm_byte, is_seq_frame=True, is_seq_pixel=True):
  ...
  image_index = 0
  save_config(image_index, is_seq_frame, is_seq_pixel, HIDE_TEMP_FOLDER)
...
def extract_secret(stegano_video_dir, key, output_file_name='video_extraction_result'):
  ...
  image_index = 0
  is_seq_frame, is_seq_pixel = extract_config(image_index, EXTRACT_TEMP_FOLDER)
  ...
  ledcn_message, len_total_message, extension, range_message = get_extension_len_message(info_image, pixel_order, BYTE_PER_PIXEL, LSM_BYTE, EXTRACT_TEMP_FOLDER)
...
def save_config(image_index, frame_sequencial, pixel_sequencial, temp_folder) :
  image = cv2.imread(temp_folder + "/" + str(image_index) + ".png", 1 )
  int_lsb = image[0,0]
  byte_lsb = ['{0:08b}'.format(x) for x in int_lsb]
