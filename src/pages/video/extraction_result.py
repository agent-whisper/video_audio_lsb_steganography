...
class VideoExtractionResult(tk.Frame):
    def __init__(self, master, output_result={'result':'failed'}):
        tk.Frame.__init__(self, master)
        self.TITLE_ROW = 0
        self.RESULT_ROW = 1
        # Judul halaman
        tk.Label(self, text='Hasil Ekstraksi Video', font='none 24 bold').grid(row=self.TITLE_ROW, column=0, columnspan=2, sticky=tk.W+tk.E)
    ...
    def render_result_frame(self, output_result):
        ...
        try:
            if output_result['result'] == 'failed':
                ...
                tk.Label(master=rr_frame, text='Ekstraksi berhasil [result:{}]'.format(output_result['result'])).grid(row=0, column=0)
